...
public class Tutorial {
   ...
   // private boolean rightIsFading = false;
    ...
    public Tutorial() {
        ...
        leftScreen.setSize(VIR_WIDTH / 2, VIR_HEIGHT);
        leftScreen.setPosition(0, 0);
        leftScreen.setColor(Color.WHITE);
        leftScreen.setAlpha(0.5f);

        rightScreen = new Sprite(Res.wallRegion);
        rightScreen.setSize(VIR_WIDTH / 2, VIR_HEIGHT);
        rightScreen.setPosition(VIR_WIDTH / 2, 0);
        rightScreen.setColor(Color.WHITE);
        rightScreen.setAlpha(0f);

        leftHand = new Sprite(Res.tapRegion);
        leftHand.setPosition(VIR_WIDTH * 0.25f - leftHand.getWidth() / 2, VIR_HEIGHT * 0.15f);
        leftHand.setColor(Color.BLACK);
        leftHand.flip(true, false);

        rightHand = new Sprite(Res.tapRegion);
        rightHand.setColor(Color.BLACK);
        rightHand.setPosition(VIR_WIDTH * 0.75f - rightHand.getWidth() / 2, VIR_HEIGHT * 0.15f);

        leftText1 = new PermanentText("hold", VIR_WIDTH * 0.25f, VIR_HEIGHT * 0.14f, "64");
        leftText2 = new PermanentText("charge", VIR_WIDTH * 0.25f, VIR_HEIGHT * 0.11f, "64");
        leftText3 = new PermanentText("release", VIR_WIDTH * 0.25f, VIR_HEIGHT * 0.08f, "64");

        rightText1 = new PermanentText("hold", VIR_WIDTH * 0.75f, VIR_HEIGHT * 0.14f, "64");
        rightText2 = new PermanentText("charge", VIR_WIDTH * 0.75f, VIR_HEIGHT * 0.11f, "64");
        rightText3 = new PermanentText("release", VIR_WIDTH * 0.75f, VIR_HEIGHT * 0.08f, "64");
    }
